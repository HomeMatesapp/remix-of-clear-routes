...
Deno.test("photographer — strong-portfolio freelance evaluates under Deno", () => {
  ...
  assert(r.considerations.some((c: string) => c.toLowerCase().includes("no mandatory qualification")));
});
...
Deno.test("photographer — no portfolio and minimal budget blocks freelance but not study", () => {
  ...
  const study = r.routes.find((x: RouteEvaluation) => x.routeId === "formal_study_plus_portfolio")!;
  assert(study.classification !== "not_currently_available_to_you");
  const freelance = r.routes.find((x: RouteEvaluation) => x.routeId === "portfolio_led_freelance")!;
