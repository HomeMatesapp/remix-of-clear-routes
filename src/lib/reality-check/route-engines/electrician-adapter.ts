...
const bestRouteForOutcome = (
  ...
  if (out.status === "route_recommended" && out.recommendedRouteId) {
    ...
    if (best.affordability.affordable) {
      whyThisFits.push("This route appears structurally suitable for you based on your answers.");
    } else {
      whyThisFits.push(
  ...
  // Verification / bridging / insufficient — no route claimed.
  const title =
...
const backupRouteForOutcome = (
  ...
  const altId: ElectricianRouteId | undefined = out.alternativeRouteIds[0];
  if (altId) {
...
const firstMovesFor = (out: ElectricianEngineOutput): string[] => {
...
export const buildElectricianResult = (
